...
function SharpCoverageBanner({ coverage, bets }: { coverage: SharpCoverage; bets?: EvBet[] }) {
  ...
  return (
    <div className="space-y-2">
      ...
      {warnings.map(({ label, count, marketKey }) => (
        <div key={marketKey} className="flex items-start gap-3 rounded-lg border border-amber-500/40 bg-amber-500/10 px-4 py-3 text-sm text-amber-300">
          <AlertTriangle className="w-4 h-4 shrink-0 mt-0.5 text-amber-400" />
          <span>
